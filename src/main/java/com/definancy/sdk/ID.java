...
public class ID {
    ...
    public ID(final byte[] idBytes) {
        ...
        if (idBytes.length != LEN_BYTES) {
            throw new IllegalArgumentException(
                    String.format("Given ID length is not %s", LEN_BYTES));
        }
    ...
    public ID(final String idStr) {
        ...
        if (idStr.length() != EXPECTED_STR_ENCODED_LEN) {
            throw new IllegalArgumentException(
                    String.format("Given ID length is not %s", EXPECTED_STR_ENCODED_LEN));
        }

        byte[] idBytes = Encoder.decodeFromBase32(idStr);
        if (idBytes.length != LEN_BYTES + CHECKSUM_LEN_BYTES) {
            throw new IllegalArgumentException(
                    String.format("Given ID length is not %s", LEN_BYTES + CHECKSUM_LEN_BYTES));
        }

        final byte[] checksum =
                Arrays.copyOfRange(idBytes, LEN_BYTES, LEN_BYTES + CHECKSUM_LEN_BYTES);
    ...
    /**
     * encodeAsString converts the ID to a human-readable representation, with a 4-byte checksum
     * appended at the end, using SHA256.
     */
